...
//FLASH写入
int Flash_writeData(uint32_t addr,uint32_t len,uint16_t *buff)   
{
	uint32_t new_addr;
    	fmc_unlock();
	for(int i=0;i<len;i++)
	{
		new_addr = addr+i*2;
		if(FMC_READY != fmc_halfword_program(new_addr,buff[i]))
		{
			return -1;
		}
		fmc_flag_clear(FMC_FLAG_END);
...
//FLASH读取
int Flash_ReadData(uint32_t addr,uint32_t len,uint16_t *buff)  //读数据
{                           
	uint32_t new_addr;
	for(int i=0;i<len;i++)
	{
...
void internal_flash_erase(uint32_t addr)
{
	...
	fmc_flag_clear(FMC_FLAG_END); //清除FMC忙碌标志位
	fmc_flag_clear(FMC_FLAG_WPERR); // 清除写保护错误标志
	fmc_flag_clear(FMC_FLAG_PGSERR);// 清除编程序列错误标志
	fmc_flag_clear(FMC_FLAG_PGMERR);// 清除编程错误标志
