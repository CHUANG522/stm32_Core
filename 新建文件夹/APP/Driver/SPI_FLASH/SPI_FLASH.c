...
void spi_flash_init(void)
{
    ...
    rcu_periph_clock_enable(RCU_SPI2);
	 ...
	 /* SPI2_CLK(PB3), SPI2_MISO(PB4), SPI2_MOSI(PB5) */
...
/*!
    ...
    \retval     none
*/
// 第一步：把这两个数组放到函数外面，变成全局变量（解决栈溢出）
uint8_t flash_buf1[4096];
uint8_t flash_buf2[4096];

void spi_flash_buffer_erase(uint32_t sector_addr,  uint32_t num_byte_to_erase)
{
	uint8_t addr = 0, count = 0;
    uint32_t num_of_sector = 0, num_of_single = 0;

    addr          = sector_addr % 4096;
    count         = 4096 - addr;
    num_of_sector = num_byte_to_erase / 4096;
    num_of_single = num_byte_to_erase % 4096;
    ...
    if(0 == addr)
	{
		while(num_of_sector-- )
		{
			...
			sector_addr += 4096;   // ? 原来错在这里！改成 +4096
		}
		if(0 != num_of_single)
		{
			spi_flash_buffer_read(flash_buf1, sector_addr + num_of_single, 4096 - num_of_single);
			spi_flash_sector_erase( sector_addr );
			spi_flash_buffer_write(flash_buf1, sector_addr + num_of_single, 4096 - num_of_single);	
		}
    }
	else
	{
		...
		{
			spi_flash_buffer_read(flash_buf1, sector_addr, addr);
			spi_flash_buffer_read(flash_buf2, sector_addr + addr + num_byte_to_erase, 4096 - addr - num_byte_to_erase);
			spi_flash_sector_erase( sector_addr );
			spi_flash_buffer_write(flash_buf1, sector_addr, addr);
			spi_flash_buffer_write(flash_buf2, sector_addr + addr + num_byte_to_erase, 4096 - addr - num_byte_to_erase);
		}
		else
		{
			spi_flash_buffer_read(flash_buf1, sector_addr, addr);
